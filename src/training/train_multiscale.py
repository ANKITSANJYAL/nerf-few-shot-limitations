...
# Import our modules
from multi_scale_dino import MultiScaleDINOFeatures
...
class MultiScaleNeRFDINOTrainer:
    def __init__(self, config):
        self.config = config
        self.device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
        
        # Initialize multi-scale DINO model
        ...
        self.nerf_model = NeRFWithDINO(
            ...
            dino_dim=128,  # Multi-scale features are 128-dimensional
        ...
        for name, param in self.dino_model.named_parameters():
            if 'lora' in name.lower() or 'feature_fusion' in name or 'cross_scale_attention' in name or 'final_proj' in name:
    ...
    def load_data(self, data_path, split='train', max_views=5):
        ...
        # Correctly handle the (N, C, H, W) tensor from data_loader
        ...
        # Precompute multi-scale DINO features for all views
        self.precompute_dino_features()
        
    def precompute_dino_features(self):
        """Extract multi-scale DINO features for all training views"""
        print("Precomputing multi-scale DINO features...")
        ...
        with torch.no_grad():
            for i, img_tensor in enumerate(self.images):
                ...
                features = self.dino_model([img_pil])  # (1, H_patch, W_patch, 128)
                features = features.squeeze(0)  # Remove batch dimension
                
                self.dino_features.append(features)
                print(f"✅ View {i+1}: Multi-scale DINO features shape {features.shape}")
    ...
    def render_rays(self, rays_o, rays_d, view_idx, N_samples=64):
        ...
        # Sample multi-scale DINO features at projected points
    ...
    def train(self, epochs):
        """Main training loop"""
        print(f"Starting multi-scale training for {epochs} epochs...")
        ...
        print(f"✅ Multi-scale training completed! Best PSNR: {self.best_psnr:.2f}")
...
def main():
    # Configuration for multi-scale approach
    config = {
        # Model parameters
        ...
        # Logging
        'output_dir': 'outputs/multi_scale_nerf_dino',
        'val_freq': 10,
        'save_freq': 50,
    ...
    # Initialize trainer
    trainer = MultiScaleNeRFDINOTrainer(config)
