...
class SpatialDINOFeatures(nn.Module):
    ...
    def __init__(self, model_name="facebook/dinov2-base", use_lora=True, lora_rank=16, lora_alpha=16, image_size=128, pos_embed_dim=64):
        super().__init__()
        ...
        # Inject LoRA into attention layers if enabled
        if use_lora:
            self._inject_lora(lora_rank, lora_alpha)
        
        # Learnable 2D positional encoding for spatial awareness
        num_patches_side = image_size // self.patch_size
        num_patches = num_patches_side * num_patches_side
        self.spatial_pos_embed = nn.Parameter(torch.randn(1, num_patches, pos_embed_dim))
        
        # Feature projection for NeRF compatibility
        self.feature_proj = nn.Sequential(
            nn.Linear(self.embed_dim + pos_embed_dim, 256),
    ...
    def sample_features_at_points(self, features, points_2d):
        ...
        """
        B, H_patches, W_patches, self.feature_dim = features.shape
